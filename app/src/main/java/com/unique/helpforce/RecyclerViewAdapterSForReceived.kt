...
class RecyclerViewAdapterSForReceived(
    val mContext: Context,
    val fragmentAdapter: Fragment,
    val UserList: List<ServiceRequests>
) : RecyclerView.Adapter<RecyclerViewAdapterSForReceived.MyViewHolder>()  {
    ...
    override fun onBindViewHolder(holder: MyViewHolder, position: Int) {
        ...
        textViewName.text = "${UserList[position].finderId.firstName}  ${UserList[position].finderId.lastName}"
        ...
        Picasso.with(mContext)
            .load(UserList[position].finderId.image)
            .into(imgViewPhoto);
        ...
        holder.DialogProfile.setOnClickListener {
            if (textViewHelpStatus.text == "Accepted") {
                ...
                Dialog_name.text =
                    "${UserList[position].finderId.firstName}  ${UserList[position].finderId.lastName}"
 //               Dialog_helptype.text = UserList[position].description
                Dialog_helpstatus.text = UserList[position].status
                Picasso.with(mContext)
                    .load(UserList[position].finderId.image)
                    .into(Dialog_image);
                Dialog_phone.text = UserList[position].finderId.phone
                ...
                Dialog_report.setOnClickListener {
                    ...
                    editor.putString(
                        "CoordLong",
                        UserList[position].finderId.location.coordinates[0].toString()
                    )
                    editor.putString(
                        "CoordLat",
                        UserList[position].finderId.location.coordinates[1].toString()
                    )
                    editor.putString(
                        "id",
                        UserList[position].finderId._id.toString()
                    )
                    editor.putBoolean(
                        "ShowOther", true
                    )
